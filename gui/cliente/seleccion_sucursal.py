...
class SeleccionSucursal:
    def __init__(self, parent):
        ...
        ttk.Label(
            main_frame,
            text="Seleccione una sucursal para la entrega:",
            font=("Helvetica", 12),
        ).pack(pady=5)

        # --- Treeview para sucursales ---
        self.tree = ttk.Treeview(
            main_frame, columns=("Nombre", "Dirección", "Distancia"), show="headings"
        )
        ...
        ttk.Button(btn_frame, text="Confirmar Selección", command=self._confirmar).pack(
            side=tk.LEFT, padx=10
        )
        ttk.Button(
            btn_frame, text="Seleccionar Más Cercana", command=self._seleccionar_cercana
        ).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text="Cancelar", command=self._cancelar).pack(
            side=tk.LEFT, padx=10
        )
    ...
    def _cargar_sucursales(self):
        ...
        for suc in sucursales:
            ...
            self.tree.insert(
                "", tk.END, values=(suc[1], suc[2], f"{suc[3]:.2f} km"), iid=suc[0]
            )
    ...
    def _confirmar(self):
        ...
        if not selected_items:
            messagebox.showwarning(
                "Advertencia", "Debe seleccionar una sucursal.", parent=self.dialog
            )
            return
    ...
    def _seleccionar_cercana(self):
        ...
        if sucursal:
            ...
            messagebox.showinfo(
                "Info",
                f"Se ha seleccionado la sucursal más cercana: {sucursal[1]}",
                parent=self.dialog,
            )
            self.dialog.destroy()
        else:
            messagebox.showerror(
                "Error", "No se encontraron sucursales.", parent=self.dialog
            )
