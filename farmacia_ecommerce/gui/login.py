...
class LoginWindow:
    ...
    def __init__(self, root):
        ...
        # Widgets
        ttk.Label(main_frame, text="Usuario:", font=('Helvetica', 12)).grid(row=0, column=0, sticky=tk.W, pady=5)
        self.username_entry = ttk.Entry(main_frame, width=25)
        self.username_entry.grid(row=0, column=1, sticky=tk.EW)

        ttk.Label(main_frame, text="Contraseña:", font=('Helvetica', 12)).grid(row=1, column=0, sticky=tk.W, pady=5)
        self.password_entry = ttk.Entry(main_frame, show="*", width=25)
        self.password_entry.grid(row=1, column=1, sticky=tk.EW)

        login_button = ttk.Button(main_frame, text="Iniciar Sesión", command=self._handle_login)
    ...
    def _handle_login(self):
        ...
        if user_data:
            self.user_info = {'id': user_data[0], 'username': user_data[1], 'rol': user_data[3]}
