...
class ProcesoCompra:
    ...
    def __init__(self, root, user_info):
        ...
        products_frame = ttk.LabelFrame(
            main_frame, text="Productos Disponibles", padding="10"
        )
        ...
        self.products_tree = ttk.Treeview(
            products_frame, columns=("ID", "Nombre", "Precio", "Stock"), show="headings"
        )
        ...
        ttk.Button(
            products_frame, text="Agregar al Carrito", command=self._agregar_al_carrito
        ...
        ttk.Button(
            cart_btn_frame, text="Eliminar Producto", command=self._eliminar_del_carrito
        ).pack(side="left", padx=5)
        ttk.Button(
            cart_btn_frame, text="Vaciar Carrito", command=self._vaciar_carrito
        ).pack(side="left", padx=5)
        ...
        self.total_label = ttk.Label(
            summary_frame, text="Total: 0.00 €", font=("Helvetica", 14, "bold")
        )
        ...
        ttk.Radiobutton(
            payment_frame, text="Contado", variable=self.metodo_pago, value="Contado"
        ...
        ttk.Style().configure(
            "Accent.TButton", font=("Helvetica", 12, "bold"), padding=10
        )

        # Botón Volver
        ttk.Button(main_frame, text="Volver al Menú", command=self._volver).grid(
        ...
        for prod in productos:
            if search_term.lower() in prod[1].lower():
                self.products_tree.insert(
                    "",
                    tk.END,
                    values=(prod[0], prod[1], f"{prod[3]:.2f}", prod[4]),
                    iid=prod[0],
                )
    ...
    def _agregar_al_carrito(self):
        ...
        cantidad = simpledialog.askinteger(
            "Cantidad",
            f"¿Cuántas unidades de '{producto[1]}' desea?",
            parent=self.root,
            minvalue=1,
            maxvalue=producto[4],
        )
    ...
    def _actualizar_carrito_treeview(self):
        ...
        for prod_id, data in self.carrito.items():
            ...
            self.cart_tree.insert(
                "",
                tk.END,
                values=(nombre, cantidad, f"{precio:.2f}", f"{subtotal:.2f}"),
                iid=prod_id,
            )
            total += subtotal
    ...
    def _eliminar_del_carrito(self):
        ...
        if not selected_items:
            messagebox.showwarning(
                "Advertencia", "Seleccione un producto del carrito para eliminar."
            )
            return
    ...
    def _vaciar_carrito(self):
        if messagebox.askyesno(
            "Confirmar", "¿Está seguro de que desea vaciar el carrito?"
        ):
            self.carrito.clear()
            self._actualizar_carrito_treeview()
    ...
    def _finalizar_compra(self):
        ...
        for prod_id, data in self.carrito.items():
            detalles.append(
                {"id_producto": prod_id, "cantidad": data[1], "subtotal": data[3]}
            )
